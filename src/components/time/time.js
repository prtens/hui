...
export const getRelativeTime = (timeStamp, locale) => {
  ...
  let resStr = "";
  let dirStr = IS_EARLY
    ? locale("h.time.before") || "前"
    : locale("h.time.after") || "后";

  if (diff < 1000) {
    resStr = locale("h.time.just") || "刚刚";
  } else if (diff < 60000) {
    // 少于等于59秒
    resStr =
      parseInt(diff / 1000) + (locale("h.time.seconds") || "秒") + dirStr;
  } else if (diff >= 60000 && diff < 3600000) {
    // 多于59秒，少于等于59分钟59秒
    resStr =
      Math.floor(diff / 60000) + (locale("h.time.minutes") || "分钟") + dirStr;
  } else if (diff >= 3600000 && diff < 86400000) {
    // 多于59分钟59秒，少于等于23小时59分钟59秒
    resStr =
      Math.floor(diff / 3600000) + (locale("h.time.hours") || "小时") + dirStr;
  } else if (diff >= 86400000 && diff < 2623860000) {
    // 多于23小时59分钟59秒，少于等于29天59分钟59秒
    resStr =
      Math.floor(diff / 86400000) + (locale("h.time.days") || "天") + dirStr;
